  ...
  if (source && typeof source === "object" && "_type" in source) {
    switch ((source as { _type: string })._type) {
      case "composite2d": {
        ...
        return {
          x:
            (devices.keyboard.isPressed(s.right) ? 1 : 0) -
            (devices.keyboard.isPressed(s.left) ? 1 : 0),
          y:
            (devices.keyboard.isPressed(s.down) ? 1 : 0) -
            (devices.keyboard.isPressed(s.up) ? 1 : 0),
        };
      }
      ...
      case "gesture:rotate": {
        ...
        return devices.touch.isGestureActive(gs) ? devices.touch.getGestureValue(gs) : false;
      }

      // Virtual control sources
      case "virtual:joystick":
        return (
          devices.virtualControls?.getJoystickValue((source as VirtualSource).id) ?? { x: 0, y: 0 }
        );
