...
function makeConfig(overrides: Partial<NormalizedDevOverlayConfig> = {}): NormalizedDevOverlayConfig {
  return {
    position: 'top-left',
    showDevices: true,
    showContexts: true,
    ...
    players: [
      {
        ...
        actions: [
          { name: 'jump', type: 'button', value: true, isJustTriggered: true, isJustReleased: false, holdTime: 0 },
          { name: 'move', type: 'axis2d', value: { x: 0.5, y: -0.5 }, isJustTriggered: false, isJustReleased: false },
          { name: 'throttle', type: 'axis1d', value: 0.8, isJustTriggered: false, isJustReleased: false },
          { name: 'idle', type: 'button', value: false, isJustTriggered: false, isJustReleased: false, holdTime: 0 },
        ],
...
function makeDebugAPI(onFrameImpl?: (cb: (snap: InputDebugSnapshot) => void) => () => void): InputDebugAPI {
  ...
  return {
    ...
    onFrame: vi.fn().mockImplementation(onFrameImpl ?? ((cb) => {
      frameCallback = cb
      return () => { frameCallback = null }
    })),
    // Expose for testing
    _triggerFrame: (snap: InputDebugSnapshot) => { frameCallback?.(snap) },
...
describe('DevOverlay', () => {
  ...
  it('shows (none active) when no active actions', () => {
    ...
    snap.players[0].actions = [
      { name: 'jump', type: 'button', value: false, isJustTriggered: false, isJustReleased: false, holdTime: 0 },
    ]
    const onFrameCb = (api.onFrame as ReturnType<typeof vi.fn>).mock.calls[0][0]
    onFrameCb(snap)
  ...
  it('shows isJustReleased indicator', () => {
    ...
    snap.players[0].actions[0] = {
      name: 'jump', type: 'button', value: true, isJustTriggered: false, isJustReleased: true, holdTime: 100
    }
    const onFrameCb = (api.onFrame as ReturnType<typeof vi.fn>).mock.calls[0][0]
    onFrameCb(snap)
  ...
  it('filters axis2d actions with x=0 and y!=0 (OR short-circuit)', () => {
    ...
    const snap = makeSnapshot({
      players: [{
        ...
        activeContexts: ['gameplay'],
        actions: [
          { name: 'move', type: 'axis2d', value: { x: 0, y: 0.5 }, isJustTriggered: false, isJustReleased: false },
        ],
        recording: { state: 'idle', frame: 0, totalFrames: 0 },
      }],
    })
