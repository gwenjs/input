...
export class DevOverlay {
  ...
  private _render(snap: InputDebugSnapshot): void {
    ...
    if (playerSnap) {
      if (cfg.showContexts) {
        const ctxStr = playerSnap.activeContexts.length > 0
          ? playerSnap.activeContexts.join(', ')
          : '(none)'
        lines.push(
          `<span style="color:#aaa">─ P${playerIdx} contexts ─</span>`,
          escHtml(ctxStr),
        )
      }
      ...
      if (cfg.showActions) {
        ...
        if (activeActions.length === 0) {
          ...
          for (const a of activeActions) {
            ...
            lines.push(`${escHtml(a.name)}: <span style="color:#ff0">${escHtml(valStr)}</span>${triggered}${released}`)
          }
        }
      }
      ...
      if (cfg.showRecording) {
        ...
        const stateColor = r.state === 'recording' ? '#f44' : r.state === 'playing' ? '#4f4' : '#aaa'
        lines.push(
