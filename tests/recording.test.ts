...
describe('InputPlayback', () => {
  ...
  it('unsubscribes callback', () => {
    ...
    let frameCount = 0
    const unsubscribe = playback.onFrame(() => { frameCount++ })
...
describe('InputPlayback - frame advance branch coverage', () => {
  ...
  it('_defaultValue returns {x:0,y:0} for axis2d action with no prior recorded value', () => {
    ...
    const binding = bind(action, { _type: 'composite2d' as const, up: 'KeyW', down: 'KeyS', left: 'KeyA', right: 'KeyD' })
    const { player: p2, context: ctx2 } = makePlayer(1)
...
describe('InputRecording.fromJSON - additional error branches', () => {
  ...
  it('throws when frame element is not an object', () => {
    expect(() => InputRecording.fromJSON({ ...base, frames: [42] })).toThrow(
      /must be an object/,
    )
  })
...
describe('InputRecording.fromJSON - object/null/invalid validation', () => {
  ...
  it('throws when frameCount is negative', () => {
    expect(() => InputRecording.fromJSON({
      version: 1, frameCount: -1, targetFps: 60, playerCount: 1, actionNames: [], frames: [],
    })).toThrow(/frameCount/)
  })

  it('throws when playerCount is zero', () => {
    expect(() => InputRecording.fromJSON({
      version: 1, frameCount: 0, targetFps: 60, playerCount: 0, actionNames: [], frames: [],
    })).toThrow(/playerCount/)
  })

  it('throws when actionNames contains non-string', () => {
    expect(() => InputRecording.fromJSON({
      version: 1, frameCount: 0, targetFps: 60, playerCount: 1, actionNames: [42], frames: [],
    })).toThrow(/actionNames/)
  })
})
