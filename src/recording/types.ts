...
export namespace InputRecording {
  ...
  export function fromJSON(raw: unknown): InputRecording {
    ...
    if (
      ...
      throw new Error(
        "[@gwenjs/input] InputRecording.fromJSON: frameCount must be a non-negative integer",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "[@gwenjs/input] InputRecording.fromJSON: playerCount must be a positive integer",
      );
    }
    ...
    const frames: InputRecordingFrame[] = (r["frames"] as unknown[]).map((f, fi) => {
      ...
      if (typeof fr["index"] !== "number") {
        throw new Error(
          `[@gwenjs/input] InputRecording.fromJSON: frame[${fi}].index must be a number`,
        );
      }
      if (!Array.isArray(fr["changes"])) {
        throw new Error(
          `[@gwenjs/input] InputRecording.fromJSON: frame[${fi}].changes must be an array`,
        );
      }
      const changes: InputRecordingChange[] = (fr["changes"] as unknown[]).map((c, ci) => {
        if (typeof c !== "object" || c === null) {
          throw new Error(
            `[@gwenjs/input] InputRecording.fromJSON: frame[${fi}].changes[${ci}] must be an object`,
          );
        }
        const ch = c as Record<string, unknown>;
        if (typeof ch["player"] !== "number") {
          throw new Error(
            `[@gwenjs/input] InputRecording.fromJSON: changes[${ci}].player must be a number`,
          );
        }
        if (ch["player"] < 0 || ch["player"] >= (r["playerCount"] as number)) {
          throw new Error(
            `[@gwenjs/input] InputRecording.fromJSON: change.player ${ch["player"] as number} out of bounds`,
          );
        }
        if (
          typeof ch["actionIndex"] !== "number" ||
          ch["actionIndex"] < 0 ||
          ch["actionIndex"] >= actionCount
        ) {
          throw new Error(
