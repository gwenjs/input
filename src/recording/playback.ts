...
export class InputPlayback {
  ...
  play(): void {
    if (!this._recording) {
      throw new Error(
        "[@gwenjs/input] InputPlayback.play(): no recording loaded — call load() first",
      );
    }
    this._state = "playing";
  ...
  private _applyFrame(frame: InputRecordingFrame): void {
    for (const change of frame.changes) {
      ...
      this._prevAccumulated[pi].set(
        change.actionIndex,
        this._accumulated[pi].get(change.actionIndex) ?? _defaultValue(change.value),
      );
