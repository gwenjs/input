...
describe("VirtualControlsOverlay", () => {
  ...
  describe("with forceVirtualControls=true", () => {
    ...
    it("creates joystick with custom position", () => {
      const forced = new VirtualControlsOverlay(true);
      forced.addJoystick(
        makeJoystickConfig({ id: "custom", side: "custom", position: { x: 20, y: 60 } }),
      );
      forced.attach();
...
describe("VirtualControlsOverlay - touch interactions", () => {
  ...
  function makeTouchList(...touches: Array<{ id: number; x: number; y: number }>): Touch[] {
...
describe("VirtualControlsOverlay - additional branch coverage", () => {
  ...
  function makeTL(...touches: Array<{ id: number; x: number; y: number }>): Touch[] {
    return touches.map((t) => ({
      identifier: t.id,
      clientX: t.x,
      clientY: t.y,
  ...
  it("joystick handleTouchEnd early returns when activeTouchId is null", () => {
    ...
    const endEvent = new TouchEvent("touchend", {
      ...
      touches: emptyList,
      targetTouches: emptyList,
      changedTouches: makeTL({ id: 1, x: 10, y: 10 }),
    });
    ...
    expect(() =>
      baseEl.dispatchEvent(
        new TouchEvent("touchmove", {
          bubbles: true,
          cancelable: true,
          touches: otherTouches,
          targetTouches: otherTouches,
          changedTouches: otherTouches,
        }),
      ),
    ).not.toThrow();
  });
  ...
  it("button handleTouchEnd early returns when activeTouchId is null", () => {
    ...
    const endEvent = new TouchEvent("touchend", {
      ...
      touches: emptyList,
      targetTouches: emptyList,
      changedTouches: makeTL({ id: 1, x: 200, y: 200 }),
    });
  ...
  it("button touchstart is ignored when button already has active touch", () => {
    ...
    buttonEl.dispatchEvent(
      new TouchEvent("touchstart", {
        ...
        targetTouches: secondTouches,
        changedTouches: secondChanged,
      }),
    );
...
describe("VirtualControlsOverlay - custom side and touchmove branches", () => {
  ...
  it("joystick handleTouchMove early returns when activeTouchId is null", () => {
    ...
    const makeTL = (...touches: Array<{ id: number; x: number; y: number }>): Touch[] =>
      touches.map((t) => ({
        identifier: t.id,
        clientX: t.x,
        clientY: t.y,
