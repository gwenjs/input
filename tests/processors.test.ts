...
describe('Smooth processor', () => {
  ...
  it('smooths scalar value over time', () => {
    const result1 = pipeline.process(1.0, [Smooth(0.5, 0)])
    const result2 = pipeline.process(1.0, [Smooth(0.5, 0)])
  ...
  it('smooths 2D vector components independently', () => {
    const result1 = pipeline.process({ x: 1.0, y: 1.0 }, [Smooth(0.5, 0)])
  ...
  it('uses independent state for different processor indices', () => {
    const smoothA = Smooth(0.5, 0)
    const smoothB = Smooth(0.5, 1)
  ...
  it('reaches target value with factor 1.0', () => {
    const result = pipeline.process(1.0, [Smooth(1.0, 0)])
    expect(result).toBe(1.0)
  })

  it('smooths minimally with factor close to 0', () => {
    pipeline.process(1.0, [Smooth(0.01, 0)])
    const result = pipeline.process(5.0, [Smooth(0.01, 0)])
